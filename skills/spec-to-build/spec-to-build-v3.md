...
1. **Read the spec.** Read the file at the given path. Understand what is being asked for — features, changes, goals, constraints.
   ...
   **Subfolder awareness:** If the spec lives in a subdirectory of `specs/` (e.g., `specs/capture_transplant/step_01.md` rather than `specs/foo.md`):
   ...
   - Scan the directory for completed step artifacts — buildplans (`*_buildplan.md`), summaries (`*_summaries.md`), and misalignment files (`*_misalignments.md`) from previous steps. Read them. They tell you what's already been built. Don't re-plan completed work, and use previous target state summaries as the current step's starting-point context.
