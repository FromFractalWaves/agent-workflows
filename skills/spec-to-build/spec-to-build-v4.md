...
1. **Read the spec.** Read the file at the given path. Understand what is being asked for — features, changes, goals, constraints.
   ...
   **Subfolder awareness:** If the spec lives in a subdirectory of `specs/` (e.g., `specs/capture_transplant/step_01.md` rather than `specs/foo.md`):
   - Read `overview.md` in the same directory first, if it exists. This gives the full picture of the task and where this step fits.
   - Scan the directory for completed step artifacts — buildplans (`*_buildplan.md`), summaries (`*_summaries.md`), misalignment files (`*_misalignments.md`), and deviation files (`*_deviations.md`) from previous steps. Read them. They tell you what's already been built. Don't re-plan completed work, and use previous target state summaries as the current step's starting-point context.
   - Also read any `*_deviations.md` files from previous steps. These tell you where earlier builds diverged from their specs — use the *actually built* names, not the original spec names, when those conflict.
...
8. **Post-build verification.** After the build plan has been executed (not during planning — this step runs when the user returns after building), verify that what was built matches what the spec described. This matters most for multi-step specs where later steps reference names and interfaces from earlier ones.

   - Read back the files that were created or modified during the build
   ...
   - Flag any deviations where the built code uses different names or structures than the spec described

   Write deviations to `specs/foo_deviations.md` if any exist. Format:
   ...
   Omit this file if the build matches the spec exactly. If deviations exist, tell the user before they move to the next step — they need to decide whether to align the code to the spec or the remaining specs to the code.
...
- `specs/foo_deviations.md` — post-build deviation report (only if deviations exist, only after build)

The original spec file is left untouched.

For a spec in a subdirectory (e.g., `specs/capture_transplant/step_01.md`), the same outputs are written as siblings in that subdirectory — `step_01_buildplan.md`, `step_01_summaries.md`, etc. No change to naming convention.
...
- **Watch for scope signals.** If the generated build plan has many steps and the test suite feels thin relative to the plan's complexity, the spec likely covers too much. Flag this to the user and suggest breaking it into a multi-step subfolder.
